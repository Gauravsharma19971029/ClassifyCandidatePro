...
public class ClassifyCandidateService {
	...
	public List<AnswerVO> retriveAllGeneralQuestionsForUser(String userId) {
		...
		try {
			...
			questionVOs = getRandomElement(questionVOs,size);

			for (Question questionVO : questionVOs) {
				
				type = questionVO.getQuestiontype();
				if(type.equals(Messages.QuestionType.GENERAL_FEATURETYPE))
				{
					if(countgen >= 10)
					{
						continue;
				...
				else if(type.equals(Messages.QuestionType.APTITUDE_FEATURETYPE))
				{
					if(countapti >= 10)
					{ 
						continue;
				...
				else if(type.equals(Messages.QuestionType.TECHNICAL_FEATURETYPE))
				{
					if(counttech >= 10)
					{
						continue;
